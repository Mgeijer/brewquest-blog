...
export default function ArizonaNewsletterPreviewPage() {
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={() => {
                if (confirm('Send Arizona newsletter to all subscribers?')) {
                  fetch('/api/admin/send-approved-content', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ contentId: 'arizona-week-3' })
                  })
                  .then(response => response.json())
                  .then(data => {
                    if (data.success) {
                      alert('Arizona newsletter sent successfully!')
                    } else {
